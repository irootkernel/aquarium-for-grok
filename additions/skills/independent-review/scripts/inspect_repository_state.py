...
def head_state(repository: Path) -> dict[str, str | None | bool]:
    ...
    resolved = git_command(repository, ["rev-parse", "--verify", "HEAD^{commit}"])
    if resolved.returncode != 0:
        if symbolic.returncode != 0:
            raise InspectionError("head_unresolved", "HEAD could not be resolved")
        commit = None
        unborn = True
    else:
        try:
            ...
            raise InspectionError("head_invalid", "HEAD commit is not ASCII") from error
        unborn = False
    try:
    ...
    return {"commit": commit, "symbolic_ref": symbolic_ref, "unborn": unborn}
