...
- `SCRIPT_SURGERY` replacement bodies are pinned to the SHA-256 of the upstream function they overwrite. A pin abort means re-derive the replacement from the new body, then rotate the `upstream` digest in the surgery plan. Re-read every replaced function on each adoption even when the pin is unchanged. v0.1.15 added surgery on `inspect_global_tools.py` (`inspect_global_mcp`, `inspect_global`, `parse_arguments`, `main`) and `inspect_ouroboros.py` (`discover_homes`, `unavailable_home`, `inspect_home`, `inspect_ouroboros`); those files call helpers this edition deletes and would otherwise keep the upstream host-home discovery this edition does not read.
- Writing-skill live targets stay `~/.agents/skills/humanizer` and `~/.agents/skills/humanize-korean`. Upstream v0.1.15 diagnoses `humanize-korean` under the upstream host skill root; do not adopt that path, and do not patch an installed plugin cache to compensate.
