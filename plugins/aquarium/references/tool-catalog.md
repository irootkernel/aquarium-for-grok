...
Install the verified two-file payload at `~/.agents/skills/humanizer` only after separate approval. Compare the complete source and target trees and every digest, rejecting missing or extra paths, symlinks, invalid frontmatter, a non-v2 or mismatched version, and duplicates in other Grok or agents skill roots. Never execute Humanizer or rewrite prose during setup.
...
Require the materialized payload to contain only regular non-symlink files, the exact generated tree, and `name: humanize-korean`. Install that complete payload at `~/.agents/skills/humanize-korean` only after separate approval. Then compare every path and digest and reject duplicates in other Grok or agents skill roots. Never point the active skill target at a checkout, run the installer against the active Grok home, invoke the skill, or create `_workspace/` during setup.
...
Pin the optional `use-podway` skill independently to full commit `9014225982e4c316237e0d6e35052414d2dbc770`. This skill hotfix adds context recovery and Grok todo list coordination guidance on top of v0.2.9 without requiring a new runtime. Keep this skill pin until an explicit catalog update replaces it; a newer release, including v0.2.10, does not change the pin.
