...
- Selecting Sanho, Mulgae, Gaori, or Podway authorizes one disclosed bounded freshness comparison against its four paired-skill files from the documented `raw.githubusercontent.com` path in ephemeral storage. No separate approval is required for those exact lookups. Treat archive downloads, init, ignore edits, hook edits, global installs, provider contact, and every network operation outside these exceptions as separately approved effects. This edition does not install or diagnose Dolgorae; Independent Review runs on Grok subagents.
...
Mulgae Config v3 has two authorities. `.mulgae/config.yaml` is Git-shareable project policy; `.mulgae/local.yaml` is untracked mode-`0600` machine configuration. Keep `execution.workspace_access: none`. Ask which providers and roles to configure. Automatic provider selection still requires authenticated AGY; Kimi, Codex, and ZCode are explicit opt-ins, and bare initialization enables only the required `logic` role. Show discovered executable, launcher, data-home, and credential-home paths only in the exact private setup proposal, never in the diagnostic report.
...
For several Codex identities, keep the two authorities separate using this Config v3 shape; it is sufficient for the active setup session; Grok reloads skills from disk, so start a new session only if the replaced `/use-mulgae` skill does not appear:
...
Treat MCP as an optional, separately approved component and prefer one user-global registration in `~/.grok/config.toml` (or `$GROK_HOME/config.toml`). Merge this table while preserving unrelated servers:
...
A user-scoped server is available in every project and inherits each session's working directory, so omit `--project-root` and pin no repository path. The user configuration is machine state; it is never staged or committed. Do not run `grok mcp doctor` during setup: it starts the server.

When the user explicitly chooses repository-local scope, merge the same table into `<absolute-git-root>/.grok/config.toml` with `args = ["mcp", "--project-root", "<absolute-git-root>"]` while preserving unrelated configuration. A same-name project entry overrides the user-global entry for that project.

Show the complete diff and target scope before approval; for a local target also show whether `.grok/config.toml` is tracked. Verify three views independently without starting the server: read the user-global `[mcp_servers.mulgae]` table from `~/.grok/config.toml` (or `$GROK_HOME/config.toml`), the local table from `<absolute-git-root>/.grok/config.toml`, and report the effective registration as the local entry when one exists and the user-global entry otherwise. Honour user-scoped `disabled_mcp_servers` as `registration_disabled` even when a project table still exists. This inspector does not read `grok mcp disable` compat sources (`.mcp.json`, Claude, Cursor), `managed_config.toml`, or `requirements.toml`, and does not walk cwd-to-root for nested `.grok/config.toml` files. It reads `command` and `args` before Grok expands `${VAR}`. Paired-skill diagnosis walks only `$GROK_HOME/skills` (or `~/.grok/skills` when `GROK_HOME` is unset), `~/.agents/skills`, `~/.claude/skills`, and `~/.cursor/skills`; it does not read `[skills] paths`, `ignore`, or `disabled`, and it does not read `[compat.claude]` or `[compat.cursor]` skill toggles. An entry is configured only when it is not `enabled = false`, is not on `disabled_mcp_servers`, resolves to the selected binary, does not set `cwd` globally (and if `cwd` is present locally it must equal the canonical root), uses exact `args = ["mcp"]` globally or `mcp --project-root <canonical-root>` locally, and uses startup and tool timeouts at least as large as the proposed defaults (`startup_timeout_sec` 30 and `tool_timeout_sec` 7501); preserve any larger existing value. Absent or smaller timeout keys are `registration_mismatch`, not configured. An unreadable configuration file degrades that view rather than proving absence. A server that has not connected yet is unverified, not degraded.

Whenever a local Mulgae registration exists, ask whether that scope is intentional. If confirmed, preserve it even when global is preferred. If not, show and separately approve removal of only the local `mulgae` table through an exact TOML edit of `.grok/config.toml`. Reinspect the file afterward. Delete `.grok/config.toml` only when it has no remaining semantic content, and delete `.grok/` only when the directory is then empty. Apply the shared backup policy before removal, never remove the user-global registration as part of local cleanup, and never stage the file during setup. Repository configuration paths must be regular non-symlink paths before `mulgae doctor` or any owning CLI probe may read them.

Tell the user to press `r` in `/mcps` to refresh, or start a new session, so MCP tools appear through `search_tool` and `use_tool` (`mulgae__preflight_review`, `mulgae__start_review`, and the rest of the attached surface). The v0.1.17 lifecycle starts exactly once and awaits the same process-local invocation without transferring observer cancellation to provider execution; use the foreground path atomically when any lifecycle tool is absent. The attached MCP surface remains versioned independently; CLI fallback preflight must identify `mulgae-review-preflight.v3`.
...
Treat MCP as an optional, separately approved component and prefer one user-global registration in `~/.grok/config.toml`:
...
A user-scoped server inherits each session's working directory, so omit `--repo` and pin no repository path. The user configuration is machine state; it is never staged or committed. Do not run `grok mcp doctor` during setup.

When the user explicitly chooses repository-local scope, merge the same table into `<absolute-git-root>/.grok/config.toml` with `args = ["--repo", "<absolute-git-root>", "mcp"]`. A same-name project entry overrides the user-global entry for that project.

Show the complete diff and target scope before approval; for a local target also show whether `.grok/config.toml` is tracked. Never stage it during setup. Verify user-global, local, and effective registrations with the same three-view TOML-reading procedure used for Mulgae, including `disabled_mcp_servers` and the same bounded Grok-config-only scope (user and project `config.toml` only; not `managed_config.toml` or `requirements.toml`). The inspector reads `command` and `args` before Grok expands `${VAR}`. Global scope requires exact `args = ["mcp"]`; local scope requires exact `--repo <canonical-root> mcp`. Require a numeric `startup_timeout_sec` of at least 30 and a numeric `tool_timeout_sec` of at least 3601 so the host deadline exceeds a one-hour command and evidence finalization, while preserving any larger existing value. Absent or smaller timeout keys are `registration_mismatch`. Do not start the server or a test. Report disabled, unresolvable-command, missing or inadequate timeout, and wrong-repository entries as degraded. A server that has not connected yet is unverified, not degraded.

Whenever a local Gaori registration exists, ask whether that scope is intentional. If not, show and separately approve removal of only the local `gaori` table through an exact TOML edit of `.grok/config.toml`. Apply the same backup, empty-file, unrelated-entry preservation, and no-staging rules as Mulgae local cleanup. Never remove the user-global registration as part of local cleanup.

Tell the user to press `r` in `/mcps` to refresh, or start a new session, so Gaori tools appear through `search_tool` and `use_tool`. `await_run` observes one process-local invocation without cancelling execution when that observer ends; use `get_run` or bounded `wait_run` when the host deadline cannot safely cover terminal completion. `list_runs` is stateless and cannot recover an invocation ID or reattach a disconnected run. Every present repository configuration path that Gaori may inspect, including `.gaori/tester.yaml`, every descendant of `.gaori/tester/rules/`, and `.gaori/toolchain.yaml`, must have regular non-symlink lexical ancestry before any owning CLI probe may read it; an absent primary path stops that probe rather than consulting ambient state.

## Lora / Lore

Official source: `https://github.com/tmdgusya/lora`

Lora distributes agent skills rather than a runtime service. Configure it for the Grok user-global scope. Resolve the latest stable tag when one exists; otherwise resolve the full current `main` commit SHA and disclose that fallback before approval. Because `npx skills add <repository>#<full-sha>` treats the SHA as a branch name, prepare a temporary detached checkout at the approved commit and install from that local source instead.
...
Install the verified two-file payload at `humanizer` under `~/.agents/skills`, only after separate approval. Compare the complete source and target trees and every digest, rejecting missing or extra paths, symlinks, invalid frontmatter, a non-v2 or mismatched version, and duplicates in other agent skill roots. Never execute Humanizer or rewrite prose during setup.

## im-not-ai

Official source: `https://github.com/epoko77-ai/im-not-ai`

Resolve the supported `v2.3.2` release through official GitHub release metadata, prepare a temporary detached checkout at that exact tag, and require `HEAD` to equal the resolved release commit. Read the checked-out `install.sh` and require every write target to derive from an isolated temporary `CODEX_HOME`. The `--codex-only` installer keys on that variable, not `GROK_HOME`. Disclose the exact `./install.sh --codex-only --copy` command and temporary target and obtain separate approval for that upstream-code execution, then run it without `--force`. Reject unexpected writes outside that isolated directory, then add the checkout's root LICENSE to the generated payload.

The `--codex-only` installer materializes only `humanize-korean` from its Codex skill tree. Require the materialized payload to contain only regular non-symlink files and `name: humanize-korean`. Install that payload at `~/.agents/skills/humanize-korean` only after separate approval. Then compare every path and digest and reject duplicates in other agent skill roots. Never point the active skill target at a checkout, run the installer against an active skill root, invoke the skill, or create `_workspace/` during setup.
...
On this host the integration has two parts: the user-scoped Ouroboros skills (`interview`, `pm`, `seed`, `qa`, and the rest of the packaged set) installed under `~/.agents/skills`, which Grok reads natively, and an `ouroboros` stdio entry in `~/.grok/config.toml`. `ooo setup --runtime grok` configures Ouroboros' own runtime selection in `~/.ouroboros/config.yaml` and does not write the Grok MCP entry, so skill installation and MCP registration stay separately approved actions. Grok is runtime-only: it drives the agentic orchestrator and is not an LLM-completion backend.

Install the skills from the exact approved package. Resolve the `ouroboros/skills/` directory bundled inside the installed `ouroboros-ai` package and copy the packaged skill set into `~/.agents/skills`, preserving each skill directory's files byte-for-byte. When a target skill directory already exists, compare the complete file sets, show the diff, apply the shared backup policy, and obtain separate replacement approval.

Merge the canonical MCP entry into `~/.grok/config.toml`, preserving unrelated servers:
...
The isolated `uvx` launcher runs the MCP 2 server in its own package environment. A project-level `.grok/config.toml` entry of the same name overrides the user entry for that project; do not create one here. Do not run `grok mcp doctor` during setup.

Diagnose four independent components with `--include-ouroboros`: the CLI through `ooo --version`, the installed user-scoped skills under `~/.agents/skills` (the packaged `interview`, `pm`, `seed`, and `qa` directories), runtime configuration through the matching launcher contract, and the effective `[mcp_servers.ouroboros]` registration read from configuration files. Run `ooo mcp doctor --json` only when the registration launches the selected `ooo` executable directly. For the canonical isolated launcher, derive runtime configuration from the exact isolated command instead. MCP registration uses the same bounded Grok-config-only three-view read as Mulgae, including `disabled_mcp_servers` and the same exclusion of `managed_config.toml` and `requirements.toml`. The inspector reads `command` and `args` before Grok expands `${VAR}`.

Registration is `configured` only for a stdio entry that is not disabled and matches either the selected `ooo` executable with exactly `args = ["mcp", "serve"]` or the canonical isolated launcher selecting `grok` through `OUROBOROS_AGENT_RUNTIME` or the isolated args ending with `--runtime grok`. Registration is `missing` when neither the user nor project Grok config carries an `ouroboros` entry, and `degraded` for a disabled entry, a launcher that matches neither supported form, or an unreadable configuration file. Installed host skills are a separate component: `host_integration` is `configured` only when those four packaged skills are present as regular non-symlink installations with valid frontmatter.

Package installation, skill installation or replacement, and the MCP entry are three separate persistent mutations with separate approvals. Re-read exact targets before each approved mutation and stop if they changed.

No setup action authorizes a provider call, authentication, repository-source transmission, `auto`, `run`, `ralph`, `evolve`, Seed creation, or an Aquarium design workflow. Verify only version, installed skills, MCP runtime, effective registration, and, when safely observable, live MCP tool exposure through `search_tool`. A server added or changed mid-session is refreshed with `r` in `/mcps` or a new session; skills reload from disk.
