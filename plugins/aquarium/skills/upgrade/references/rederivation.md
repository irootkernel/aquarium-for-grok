...
- v0.1.17 excludes `tools/`, the `status` skill, the development-channel contract reference, and plugin `.mcp.json`. `aquarium-dev` and `aquarium-status` stay with the upstream edition. Do not reintroduce their inspectors.
- The route token `native-codex` stays byte-stable in procedures and evidence fields. Prose dispatches it with `spawn_subagent`. `/aquarium:independent-review` stays the standalone entrypoint and is not an in-handler route.
- `effective_codex_skill_root` keeps its name and returns the Grok host skill root. Canonical Humanizer and `humanize-korean` targets stay `~/.agents/skills`. A byte-identical Ouroboros copy there is canonical; only a conflicting copy degrades readiness.
- Ouroboros support is `>=0.51.1` with no upper bound. Do not restore `<0.54.0`.
