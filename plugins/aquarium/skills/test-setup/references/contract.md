...
Use `Contract: aquarium-test-contract/v1` and `Profile: <selected-profile>` as direct declarations in the Contract section for unambiguous machine-readable enrollment. These explicit fields are the only machine-readable enrollment authority; surrounding prose is not interpreted. The contract identifier is exact, while profile enum values are case-normalized. Inline Markdown, heading capitalization, and closing heading markers do not change these declarations. Comments, top-level fenced examples, and list-nested indented examples do not enroll the repository. The inspector checks section presence and declarations; human review checks the prose's meaning.
