...
설치, 스킬 표, 생성 모델, 업그레이드, 검증은 [영어 README](README.md)와 같습니다. 문서의 `/aquarium:<skill>`은 플러그인 한정 이름입니다. 설치 후 실제 입력은 `grok inspect --json`이 보고하는 형식(충돌이 없으면 `/<skill>`, 있으면 `/aquarium:<skill>`)을 따릅니다. Independent Review는 이 호스트에서 Grok `spawn_subagent`만 사용하며 Dolgorae를 쓰지 않습니다.
